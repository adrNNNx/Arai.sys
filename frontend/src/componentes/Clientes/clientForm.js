...
import { Button, TextField, Paper, FormHelperText, Grid, Typography } from '@mui/material';
...
import { apiUrlCreaClient, apiUrlDeleteClient, apiUrlUpdateClient } from '../../services/Apirest';
import { useAraiContext } from 'context/arai.context';

//Valores iniciales del formulario
const initialValues = { nom_per: '', tel_per: '', dire_per: '', correo_per: '', ci_cli: '' };

//Validaciones formulario
const validationSchema = Yup.object({
  nom_per: Yup.string().required('El nombre del cliente es requerido'),
  ci_cli: Yup.string().required('El CI para el cliente es requerido'),
  tel_per: Yup.string().required('El teléfono para el cliente es requerido'),
...
const ClienteForm = () => {
  ...
  useEffect(() => {
    if (araiContextValue.action === 'editar') {
      ...
      setInitialFormValues({
        ...
        ci_cli: araiContextValue.ci_cli
      });
      setEditar(true);
    } else if (araiContextValue.action === 'eliminar') {
      deleteCliente(araiContextValue);
    }
  }, [araiContextValue]);
  ...
  const addCliente = (values, resetForm) => {
    const data = {
      nom_per: values.nom_per,
      tel_per: values.tel_per,
      dire_per: values.dire_per,
      correo_per: values.correo_per,
      ci_cli: values.ci_cli
    };

    sendPostRequest(apiUrlCreaClient, data, `<i>El cliente <strong>${values.nom_per}</strong> fue registrado con éxito</i>`, () => {
      limpiarCampos();
  ...
  const updateCliente = (values, resetForm) => {
    const data = {
      id_per: id_per,
      nom_per: values.nom_per,
      tel_per: values.tel_per,
    ...
    sendPutRequest(apiUrlUpdateClient, data, `<i>El cliente <strong>${values.nom_per}</strong> fue actualizado con éxito</i>`, () => {
      limpiarCampos();
      setDataUpdateContext(true);
      resetForm();
    });
  ...
  const deleteCliente = (val) => {
    Swal.fire({
      ...
      html: `<i>¿Realmente desea eliminar al cliente <strong>${val.nom_per}</strong>?</i>`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        sendDeleteRequest(apiUrlDeleteClient, data, `<i>El cliente <strong>${val.nom_per}</strong> fue eliminado.</i>`, () => {
          limpiarCampos();
          setDataUpdateContext(true);
        });
      }
    ...
    >
      <Formik
        ...
        onSubmit={(values, { setSubmitting, resetForm }) => {
          if (editar) {
            updateCliente(values, resetForm);
          } else {
            addCliente(values, resetForm);
          }
          setSubmitting(false);
        }}
      >
          ...
          <Form>
            <Paper style={{ padding: '20px', marginBottom: '20px' }}>
              <Typography sx={{ mt: 2 }} variant="h3" id="formTitle" component="div">
                {editar ? 'Actualizar Cliente' : 'Añadir Cliente'}
              </Typography>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <Field as={TextField} name="nom_per" label="Nombre del Cliente *" fullWidth margin="normal" />
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  />
                  <ErrorMessage name="ci_cli">{(msg) => <FormHelperText error>{msg}</FormHelperText>}</ErrorMessage>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Field
                    as={TextField}
